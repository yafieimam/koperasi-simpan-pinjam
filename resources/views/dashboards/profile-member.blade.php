        ...
        <div id="tab_1-1" class="tab-pane active">
         ...
         <div class="col-md-6 no-padding col-sm-6 col-xs-6">
            ...
            @php
                ...
                if(validateDate($getData->dob)){
                    $getData->dob = now();
                }
            @endphp
             <!-- <input id="datepickerDOB" value="{{ $getData->dob }}" name="dob" title="datepicker" style="width: 100%" class="defaultHide datepicker" />  -->
             {!! Form::date('dob',$getData->dob, ['id' => 'datepickerDOB', 'class' => 'form-control datepicker col-md-7 col-xs-12 defaultHide','required'=>false]) !!}
         </div>
         <div class="col-md-6 col-sm-6 col-xs-6">
         ...
         <div class="col-md-6 no-padding col-sm-6 col-xs-6">
            ...
            <p class="show_default">{{ ucwords($getData->salary) }}</p>
            <input type="text" class="form-control defaultHide" placeholder="Gaji" name="salary" id="salary" value="{{ $getData->salary }}">
         </div>
         <div class="space"></div>
         ...
         <div class="col-md-6 no-padding col-sm-6 col-xs-6">
             ...
             <select name="position_id" id="position_id" class="form-control defaultHide" required>
             ...
             @php
                if($getData->start_date == null){
                   $getData->start_date = now();
                    ...
                    $getData->start_date = \Carbon\Carbon::parse($getData->start_date)->format('Y-m-d');
                }

                if(validateDate($getData->start_date)){
                    $getData->start_date = now();
                }
            @endphp
             <!-- <input id="datepickerDOB" value="{{ $getData->start_date }}" name="start_date" title="datepicker" style="width: 100%" class="defaultHide datepickerDOB" /> -->
             {!! Form::date('start_date',$getData->start_date, ['id' => 'start_date', 'class' => 'form-control datepicker col-md-7 col-xs-12 defaultHide','placeholder'=>'Tanggal Awal','required'=>false]) !!}
             ...
             @php
                if($getData->end_date == null){
                   $getData->end_date = now();
                    ...
                    $getData->end_date = \Carbon\Carbon::parse($getData->end_date)->format('Y-m-d');
                }

                if(validateDate($getData->end_date)){
                    $getData->end_date = now();
                }
             @endphp
             {!! Form::date('end_date',$getData->end_date, ['id' => 'start_date', 'class' => 'form-control datepicker col-md-7 col-xs-12 defaultHide','placeholder'=>'Tanggal Akhir','required'=>false]) !!}
             <!-- <input id="datepickerEnd" value="{{ $getData->end_date }}" name="end_date" title="datepicker" style="width: 100%" class="defaultHide datepickerDOB" /> -->
         </div>
         <div class="space"></div>
         <div class="col-md-6 no-padding col-sm-6 col-xs-6">
             <label for="">Status</label>
             <p class="show_default">{{ ucwords($getData->status) }}</p>
             <select name="status" id="status" class="form-control defaultHide">
         ...
         <div class="col-md-6 col-sm-6 col-xs-6">
             ...
             <select name="is_permanent" id="is_permanent" class="form-control defaultHide">
...
$('.edit-button').show();
$("#position_id").val("{{ $getData->position_id }}").change();
$("#status").val("{{ $getData->status }}").change();
$("#is_permanent").val("{{ $getData->is_permanent }}").change();
$('#salary').mask('0.000.000.000.000.000', {reverse:true});
});
$('.defaultHide').hide();
$('#is_permanent').on('change', function() {
    var permanent = $(this).val();
    console.log(permanent == 1);
    if(permanent == 1){
        $('#datepicker').val('');
        $('#datepicker1').val('');
    }else{
        $('#datepicker').val("{{ \Carbon\Carbon::parse($getData->start_date)->format('d-M-Y') }}");
        $('#datepicker1').val("{{ \Carbon\Carbon::parse($getData->end_date)->format('d-M-Y') }}");
    }
});
