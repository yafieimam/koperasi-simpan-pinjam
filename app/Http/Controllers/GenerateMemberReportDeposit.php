...
class GenerateMemberReportDeposit extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        $member_id = $request->get('member_id');
        $dataDepositMember = reverseDataHelper::generateMemberDeposit($start, $end, $member_id);
        $output = '';
        foreach ($dataDepositMember as $deposit) {
            $output .= '<tr>' .
                '<td>' . $deposit['tahun'] . '</td>' .
                '<td>' . number_format($deposit['pokok']) . '</td>' .
                '<td>' . number_format($deposit['wajib']) . '</td>' .
                '<td>' . number_format($deposit['sukarela']) . '</td>' .
                '<td>' . number_format($deposit['shu']) . '</td>' .
                '</tr>';
...
//        return $dataDepositMember;
        $filename = $member->nik_koperasi.'_'.$member->first_name.' '.$member->last_name.'_simpanan.xlsx';
