...
class GenerateReportDepositsController extends Controller
{
    ...
    {
        $indents = [];
        return view('report.generate.deposit.new', compact('indents'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $dataDeposit = DownloadReport::generateDeposit($start, $end, $area, $proyek, $member_id);
        $output = '';
        foreach ($dataDeposit['pokok'] as $key => $deposit) {
            $output .= '<tr>' .
                '<td style="text-align: center;">' . $deposit['bulan'] . '</td>' .
                '<td style="text-align: center;">' . number_format($deposit['debit']) . '</td>' .
                '<td style="text-align: center;">' . number_format($deposit['credit']) . '</td>' .
                '<td style="text-align: center;">' . number_format($dataDeposit['wajib'][$key]['debit']) . '</td>' .
                '<td style="text-align: center;">' . number_format($dataDeposit['wajib'][$key]['credit']) . '</td>' .
                '<td style="text-align: center;">' . number_format($dataDeposit['sukarela'][$key]['debit']) . '</td>' .
                '<td style="text-align: center;">' . number_format($dataDeposit['sukarela'][$key]['credit']) . '</td>' .
                '</tr>';
        }
    ...
    public function download(Request $request){
        ...
        $proyek = $request->get('proyek');
        $member_id = $request->get('member_id');
        $genReportDeposit = DownloadReport::generateDeposit($start, $end, $area, $proyek, $member_id);
        $spreadsheet = ReverseDataHelper::downloadDeposit($genReportDeposit, $start, $end);
        $filename = 'Laporan Simpanan.xlsx';
