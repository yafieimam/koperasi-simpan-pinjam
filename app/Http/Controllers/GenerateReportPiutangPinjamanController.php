...
class GenerateReportPiutangPinjamanController extends Controller
{
    /**
    ...
    {
        $indents = [];
        return view('report.generate.piutang-pinjaman.new', compact('indents'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $dataLoan = DownloadReport::generatePiutangPinjaman($start, $end, $area, $proyek, $member_id);
        $output = '';
        foreach ($dataLoan as $key => $loan) {
            $output .= '<tr>' .
                '<td style="text-align: center;">' . $loan['bulan'] . '</td>' .
                '<td style="text-align: center;">' . number_format($loan['tunai']) . '</td>' .
                '<td style="text-align: center;">' . number_format($loan['barang']) . '</td>' .
                '<td style="text-align: center;">' . number_format($loan['pendidikan']) . '</td>' .
                '<td style="text-align: center;">' . number_format($loan['softloan']) . '</td>' .
                '<td style="text-align: center;">' . number_format($loan['motorloan']) . '</td>' .
                '<td style="text-align: center;">' . number_format($loan['bisnis']) . '</td>' .
                '<td style="text-align: center;">' . number_format($loan['total']) . '</td>' .
                '</tr>';
        }
    ...
    public function download(Request $request){
        ...
        $proyek = $request->get('proyek');
        $member_id = $request->get('member_id');
        $genReportDeposit = DownloadReport::generatePiutangPinjaman($start, $end, $area, $proyek, $member_id);
        $spreadsheet = ReverseDataHelper::downloadPiutangPinjaman($genReportDeposit, $start, $end);
        $filename = 'Laporan Piutang Pinjaman.xlsx';
