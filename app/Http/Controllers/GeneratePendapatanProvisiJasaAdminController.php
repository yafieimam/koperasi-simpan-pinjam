...
class GeneratePendapatanProvisiJasaAdminController extends Controller
{
    public function index()
    {
        $indents = [];
        return view('report.generate.pendapatan-provisi-jasa-admin.new', compact('indents'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        $dataDepositMember = reverseDataHelper::generatePendapatanProvisiJasaAdmin($start, $end, $area, $proyek, $member_id);
        $output = '';
        foreach ($dataDepositMember as $deposit) {
            $output .= '<tr>' .
                '<td style="text-align: center;">' . $deposit['tahun'] . '</td>' .
                '<td style="text-align: center;">' . number_format($deposit['provisi']) . '</td>' .
                '<td style="text-align: center;">' . number_format($deposit['jasa']) . '</td>' .
                '<td style="text-align: center;">' . number_format($deposit['admin']) . '</td>' .
                '</tr>';
    ...
    public function download(Request $request){
        ...
        $member_id = $request->get('member_id');
        $dataDepositMember = reverseDataHelper::generatePendapatanProvisiJasaAdmin($start, $end, $area, $proyek, $member_id);
        $spreadsheet = DownloadReport::downloadPendapatanProvisiJasaAdmin($dataDepositMember, $start, $end);
//        return $dataDepositMember;
        $filename = 'Laporan Pendapatan Provisi Jasa dan Admin.xlsx';
